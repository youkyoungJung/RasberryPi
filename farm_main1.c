...
void  ClientRecv( int  clntSock )						
{
	...
	if(_message->_command == GET_STATUS)
		...
		//	iot_get_history_data_from_db(QUERY_SELECT_SENSOR_DATA, _result);
			iRet = sprintf (ucSBuff, "%d|%d|%s, %d, %d, %d", _message->_apptype, _message->_command, _result, get_temperature_sensor(), get_humidity_sensor(), get_light_sensor());
		...
		else if( _message->_command == WATER_LED )
		{
			...
			write( clntSock, ucSBuff, iRet );
		...
		else if(_message->_command == GET_SETTING)
				{
					...
					char * _result = (char *)malloc(sizeof(char)*4096);
					memset(_result, 0x0, 4096);
					iRet = sprintf (ucSBuff, "%d|%d|%s,%d,%d,%d,%d", _message->_apptype, _message->_command, _result, get_light_sensor(), get_temperature_sensor(), get_humidity_sensor());
					free(_result);
				}
